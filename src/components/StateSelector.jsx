...
export default function StateSelector({ onSelectState }) {
  const federalOnlyStates = ['TX', 'FL', 'NY', 'PA'];
      ...
      <div className="space-y-2">
        {IMPLEMENTED_STATES.map(code => (
          <button
            ...
            className="
              w-full p-4 border-2 border-emerald-500 bg-emerald-50 
      ...
      <div className="space-y-2">
        {federalOnlyStates.map(code => (
          <button
            ...
            onClick={() => onSelectState(code)}
            className="
              w-full p-3 border-2 border-gray-200 rounded-xl text-left 
