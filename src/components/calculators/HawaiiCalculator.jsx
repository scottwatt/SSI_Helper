...
import { HI_RATES, HI_LIVING_ARRANGEMENTS } from '../../data/hawaiiRates';
import { STATES } from '../../data/states';

export default function HawaiiCalculator() {
  ...
  const calculate = () => {
    ...
    const rateData = data.household === 'couple' 
      ? HI_RATES.couple[data.living]
      : HI_RATES.single[data.living];
    ...
    const qualifies1619b = ssiPayment === 0 && grossEarned > 0 && annualEarnings <= STATES.HI.threshold1619b;
  ...
  if (results) {
    return (
      <div className="space-y-4">
        <div className="text-center p-6 bg-gradient-to-br from-teal-600 to-cyan-700 text-white rounded-2xl shadow-lg">
          <p className="text-sm opacity-90 font-medium">Estimated Hawaii SSI Payment</p>
          <p className="text-5xl font-bold my-2">${results.ssiPayment.toFixed(2)}<span className="text-lg font-normal">/mo</span></p>
          <p className="text-teal-100">Total Monthly Income: ${results.totalIncome.toFixed(2)}</p>
        </div>

        <div className="bg-teal-50 p-4 rounded-xl border border-teal-200">
          <div className="font-semibold text-teal-900">Living Arrangement</div>
          <p className="text-teal-700 text-sm">{results.livingLabel}</p>
          <p className="text-teal-600 text-sm mt-1">Base Rate: ${results.base.toFixed(2)}/mo</p>
        </div>

        <div className="bg-blue-50 p-3 rounded-xl border border-blue-200 text-sm text-blue-800">
          <strong>Note:</strong> Hawaii's state supplement is administered by SSA, so you receive a single combined payment.
        </div>
        ...
        {results.qualifies1619b && (
          <div className="bg-purple-50 border border-purple-200 p-4 rounded-xl">
            ...
            <p className="text-purple-600 text-xs">Annual earnings below HI threshold</p>
          </div>
        )}

        <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl text-xs text-amber-800">
          <strong>Reminder:</strong> This is an estimate. Actual payments determined by SSA.
        </div>
  ...
  const questions = [
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          <p className="text-sm text-gray-600 mb-2">Select your living situation in Hawaii.</p>
          {(() => {
            const rates = data.household === 'couple' ? HI_RATES.couple : HI_RATES.single;
            return HI_LIVING_ARRANGEMENTS.map(key => (
              <RadioOption
                key={key}
                value={key}
                current={data.living}
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors ${i <= activeStep ? 'bg-teal-500' : 'bg-gray-200'}`} />
        ))}
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button onClick={handleNext} disabled={!canContinue()} className={`flex-1 py-3 rounded-xl font-medium transition-all ${canContinue() ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
